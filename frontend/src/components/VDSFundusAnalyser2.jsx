...
// v7: OOD images now only show the original photo (Grad-CAM tabs hidden) — everything else unchanged
...
/* ── Region activation bar ────────────────────────── */
function RegionBar({ zone, value, index }) {
  const pct = value * 100;
  const color =
    pct > 60 ? "#f43f5e" :
    pct > 40 ? "#f97316" :
  ...
  return (
    <div className="flex items-center gap-3"
      style={{ animation: `fadeUp 0.4s ease both`, animationDelay: `${index * 55}ms` }}>
      <span className="w-40 text-xs font-medium truncate text-slate-500 shrink-0">{zone}</span>
      <div className="flex-1 h-2 overflow-hidden rounded-full bg-slate-100">
        <div
          className="h-full transition-all duration-1000 rounded-full"
          style={{ width: `${pct}%`, background: color, transitionDelay: `${index * 60 + 200}ms` }}
        />
      </div>
      <span className="w-10 font-mono text-xs font-semibold text-right text-slate-600">
        <AnimatedNumber value={pct} decimals={2} />
      </span>
    </div>
  );
}
...
function OODBanner({ result }) {
  if (result.is_ood == null) return null; // OOD detection wasn't available on the backend

  const flagged = result.is_ood === true;
  ...
  return (
    <div
      style={{
        ...
        background: flagged ? "#fff7ed" : "#f0fdf4",
        border: `1px solid ${flagged ? "#fed7aa" : "#bbf7d0"}`,
        animation: "fadeUp 0.4s ease both",
      }}
    >
      <span style={{ fontSize: 16, lineHeight: 1, marginTop: 1 }}>{flagged ? "⚠️" : "✅"}</span>
      <div style={{ flex: 1 }}>
        <p style={{
          fontSize: 12, fontWeight: 700,
          color: flagged ? "#c2410c" : "#15803d",
          marginBottom: 2, textTransform: "uppercase", letterSpacing: "0.04em"
        }}>
          {flagged ? "Out-of-Distribution Image Detected" : "In-Distribution Image"}
        </p>
        <p style={{ fontSize: 12, color: flagged ? "#9a3412" : "#166534", lineHeight: 1.6 }}>
          {flagged
            ? "This image's learned features fall outside the range the model was trained on. Classification, VDS score, and clinical detail are withheld for out-of-distribution images."
            : "This image's learned features are consistent with the training distribution, supporting the reliability of this analysis."}
        </p>
      </div>
      {result.ood_score != null && (
        <div style={{ textAlign: "right", flexShrink: 0 }}>
          <span className="mono" style={{
            ...
            {result.ood_score.toFixed(2)}
          </span>
          <span style={{ fontSize: 9, color: "#94a3b8" }}>
            vs {result.ood_threshold != null ? result.ood_threshold.toFixed(2) : "—"} threshold
          </span>
        </div>
      )}
    </div>
...
/* ── GradCAM image tabs ───────────────────────────── */
function GradCAMPanel({ result, originalPreview }) {
  const isOod = result.is_ood === true;
  // OOD images can't be trusted to have a meaningful attention map, so we
  // only ever offer the original photo — no overlay/heatmap tabs at all.
  const [tab, setTab] = useState(isOod ? "original" : "overlay");
  ...
  const tabs = isOod
    ...
    : [
        ...
        { id: "heatmap",  label: "Activation Map" },
      ];

  const imgSrc =
    tab === "original" ? originalPreview :
    tab === "overlay"  ? `data:image/jpeg;base64,${result.gradcam_overlay}` :
                         `data:image/jpeg;base64,${result.gradcam_heatmap}`;

  return (
    <div className="card" style={{ overflow: "hidden", animation: "fadeUp 0.4s ease both" }}>
      {/* Tab bar */}
      <div style={{
        ...
        {isOod && (
          <span style={{
            ...
            padding: "2px 6px"
          }}>ATTENTION MAP UNAVAILABLE — OOD</span>
        )}

        {!isOod && result.vds_source === "seeded" && (
          <span style={{
            marginLeft: "auto", alignSelf: "center",
            fontSize: 9, fontFamily: "monospace",
      ...
      <div style={{ padding: 16 }}>
        ...
        {!isOod && tab === "heatmap" && (
          <div style={{
            ...
            padding: "6px 12px", background: "#0d0d1a", borderRadius: 8
          }}>
            <span style={{ fontSize: 10, color: "#6366f1", fontFamily: "monospace" }}>Low</span>
            <div style={{
              flex: 1, height: 8, borderRadius: 4,
              background: "linear-gradient(90deg, #00008b, #0000ff, #00ffff, #ffff00, #ff0000)"
            }} />
            <span style={{ fontSize: 10, color: "#f43f5e", fontFamily: "monospace" }}>High</span>
          </div>
        )}

        {!isOod && tab === "overlay" && (
          <p style={{ marginTop: 8, fontSize: 11, color: "#94a3b8", textAlign: "center" }}>
            55% original · 45% Grad-CAM — warm regions indicate highest model attention
          </p>
        )}
      </div>
    </div>
...
function ClinicalPanel({ clinical, cfg, result }) {
  ...
  const sections = [
    ...
    },
    {
      id: "optical",
      icon: "👁️",
      title: "Optical & Lens Analysis",
      content: clinical.optical_analysis,
      type: "kv",
      keys: { lens_status: "Lens Status", light_path: "Light Transmission", reflex: "Red Reflex" },
    },
    {
      id: "anatomical",
      icon: "🩺",
      title: "Anatomical Findings",
      content: clinical.anatomical_findings,
      type: "kv",
      keys: { vessels: "Retinal Vasculature", disc: "Optic Disc" },
    },
    {
      id: "ood",
      icon: "🛡️",
      title: "Out-of-Distribution Check",
      content: {
        note: clinical.ood_note,
    ...
    },
    {
      id: "scores",
      icon: "📊",
      title: "Quantitative Feature Analysis",
      content: clinical.score_breakdown,
      type: "scores",
    },
    {
      id: "gradcam",
      ...
      content: clinical.heatmap_interpretation,
      type: "text",
    },
    {
      id: "rec",
      icon: "📋",
      title: "Clinical Note & Recommendation",
      content: { note: clinical.clinical_note, rec: clinical.recommendation },
      type: "rec",
    },
  ];
  ...
  return (
    <div className="card" style={{ overflow: "hidden" }}>
      <div style={{
        ...
        <span className="accent-label" style={{ display: "inline" }}>Clinical Explanation</span>
      ...
      <div style={{ padding: "8px 0" }}>
        {sections.map((sec, idx) => (
          <div key={sec.id}
            style={{ borderBottom: idx < sections.length - 1 ? "1px solid #f8fafc" : "none" }}>
            {/* Accordion header */}
            <button
              ...
              <span style={{ display: "flex", alignItems: "center", gap: 8 }}>
                ...
                <span style={{ fontSize: 12, fontWeight: 600, color: "#374151" }}>{sec.title}</span>
                {sec.id === "ood" && sec.content.isOod === true && (
                  <span style={{
            ...
            {open === sec.id && (
              <div style={{
                padding: "0 20px 16px", animation: "fadeUp 0.2s ease both"
              }}>
                {sec.type === "text" && (
                  <p style={{ fontSize: 12, color: "#64748b", lineHeight: 1.75 }}>
                    {sec.content}
                  </p>
                )}
                    ...
                    {sec.content.score != null && (
                      <div style={{
                        ...
                        <span style={{
                          ...
                          color: sec.content.isOod ? "#c2410c" : "#0f172a"
                        }}>
                          {sec.content.score.toFixed(2)}
                        </span>
                        <span style={{ fontSize: 10, color: "#94a3b8", fontFamily: "monospace" }}>
                          / threshold {sec.content.threshold != null ? sec.content.threshold.toFixed(2) : "—"}
                        </span>
                      </div>
                    )}
                    <p style={{
                      fontSize: 12, lineHeight: 1.75,
                      color: sec.content.isOod ? "#c2410c" : "#64748b"
                    }}>
                      {sec.content.note}
                    </p>
                  </div>
                      ...
                      <div key={k} style={{
                        ...
                        <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 4 }}>
                          ...
                          <span style={{ display: "flex", gap: 8 }}>
                            ...
                            <span style={{
                              fontSize: 11, fontFamily: "monospace", fontWeight: 700, color: "#0f172a"
                            }}>{v.value.toFixed(1)}%</span>
                          </span>
                        </div>
                        <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                          <div style={{ flex: 1, height: 4, background: "#e2e8f0", borderRadius: 4, overflow: "hidden" }}>
                            <div style={{
                              ...
                              width: `${v.value}%`,
                              background: v.value > 60 ? "#f43f5e" : v.value > 30 ? "#f97316" : "#10b981",
                        ...
                        <p style={{ fontSize: 10, color: "#94a3b8", marginTop: 3, fontFamily: "monospace" }}>
                          {v.detail}
                        </p>
                      </div>
                    ))}
                ...
                {sec.type === "rec" && (
                  <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
                    <div style={{
                      ...
                      <span style={{
                        fontSize: 10, fontFamily: "monospace", color: "#16a34a",
                        textTransform: "uppercase", letterSpacing: "0.08em", display: "block", marginBottom: 4
                      }}>Clinical Note</span>
                      <p style={{ fontSize: 12, color: "#15803d", lineHeight: 1.65 }}>
                        {sec.content.note}
                      </p>
                    </div>
                    <div style={{
                      ...
                      <span style={{
                        fontSize: 10, fontFamily: "monospace", color: "#2563eb",
                        textTransform: "uppercase", letterSpacing: "0.08em", display: "block", marginBottom: 4
                      }}>Recommendation</span>
                      <p style={{ fontSize: 12, color: "#1d4ed8", lineHeight: 1.65 }}>
                        {sec.content.rec}
                      </p>
                    </div>
                  </div>
...
export default function VDSFundusAnalyser() {
  ...
  // Find the highest class probability to display in the VDS Hero
    ...
    <>
      <style>{`
        ...
        *, *::before, *::after { box-sizing: border-box; }
        body { font-family: 'Sora', sans-serif; background: #f6f7f9; color: #1e293b; }
        ...
        .card { background: #ffffff; border: 1px solid #e8edf3; border-radius: 16px; }
        .dot-bg {
        ...
        .gauge-track {
          ...
          background: linear-gradient(90deg,
            rgba(16,185,129,0.15) 0%, rgba(245,158,11,0.15) 40%,
            rgba(249,115,22,0.15) 70%, rgba(244,63,94,0.15) 100%
          );
          border: 1px solid #e2e8f0; overflow: hidden;
        }
        .gauge-fill { height: 100%; border-radius: 99px; transition: width 1.1s cubic-bezier(0.22, 1, 0.36, 1); }
        ...
        .analyse-btn:not(:disabled):hover {
          box-shadow: 0 6px 20px rgba(99,102,241,0.4); transform: translateY(-1px);
        }
        .analyse-btn:disabled { background: #f1f5f9; color: #cbd5e1; cursor: not-allowed; }
        .reset-btn {
        ...
        .reset-btn:hover { border-color: #cbd5e1; color: #64748b; }
        .spinner {
                    ...
                    {preview ? (
                      <div style={{ padding: 12 }}>
                        <img src={preview} alt="Fundus preview"
                          style={{ width: "100%", objectFit: "contain", borderRadius: 8, maxHeight: 180, display: "block" }} />
                        <div style={{
                ...
                <div className="card" style={{
                  ...
                  <div style={{ display: "flex", flexDirection: "column", gap: 13 }}>
                    {[
                      ...
                      ["02", "EfficientNet-B3 classifies severity (Mild/Moderate/No/Severe)"],
                      ["03", "VDS computed from 5 weighted signals with range validation"],
                      ["04", "Grad-CAM generates spatial attention heatmap"],
                      ["05", "Clinical explanation with region activations returned"],
                    ].map(([n, t]) => (
                ...
                <div className="card" style={{ padding: "20px 24px", animation: "fadeUp 0.5s ease both", animationDelay: "0.1s" }}>
                  ...
                  <div style={{
                    ...
                    {COMPONENT_META.map((meta, i) => {
                      const compValue = result[meta.key] ?? (result.components ? result.components[meta.key] : 0);
                      return <ScoreTile key={meta.key} meta={meta} value={compValue ?? 0} index={i} />;
                    })}
                  </div>
                </div>
              )} */}
            ...
            <div>
              ...
              {loading && (
                <div className="card" style={{
                  ...
                  <p style={{ fontSize: 12, color: "#94a3b8" }}>Running EfficientNet-B3 + Grad-CAM</p>
                </div>
              )}
              ...
              {result && cfg && (
                <div style={{ display: "flex", flexDirection: "column", gap: 16,
                  ...
                  {/* OOD banner — surfaces the Mahalanobis-distance check from the backend */}
                  <OODBanner result={result} />

                  {/* GradCAM panel — shows only the original image when the input is OOD */}
                  {result.gradcam_overlay && (
                    <GradCAMPanel result={result} originalPreview={preview} />
                  )}

                  {/* Class probabilities — hidden for OOD images */}
                  {!isOod && (
                  ...
                  {/* {!isOod && result.region_activations && (
                    <div className="card" style={{ padding: "20px 24px" }}>
                      ...
                      <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
                        {REGION_ORDER.map((zone, i) => {
                          const actVal = result.region_activations
                            ? (Array.isArray(result.region_activations) ? result.region_activations[i] : result.region_activations[zone])
                            : 0;
                          return (
                            <RegionBar
                              key={zone}
                      ...
                      <p style={{ fontSize: 10, color: "#cbd5e1", marginTop: 12, fontFamily: "monospace" }}>
                        Values 0–1 indicate mean activation intensity per anatomical zone.
